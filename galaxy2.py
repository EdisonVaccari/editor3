...
# Função para salvar texto como .docx
def salvar_como_docx(texto):
    # Cria um arquivo temporário para armazenar o .docx
...
with tab2:
    ...
    with col3:
        ...
        # Botão para baixar o texto em formato .docx
        if st.button("Baixar como .docx"):
            docx_path = salvar_como_docx(st.session_state["texto_gerado"])
            with open(docx_path, "rb") as file:
                st.download_button(
                    label="Clique aqui para baixar",
                    data=file,
                    file_name="texto_gerado.docx",
